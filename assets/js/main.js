...
if (!['en','fa'].includes(currentLang)) currentLang = 'en';
...
/* === Reliable English / Dari language switch override === */
(function () {
  const I18N_FIX = {
    en: {
      dir: "ltr",
      ...
      thisMonthPrincipal: "This month to principal",
      interestSavedThisMonth: "Interest saved this month",
      afterSelectedStrategy: "After selected strategy",
      interestComparisonTitle: "Interest comparison",
      interestComparisonSub: "Compare interest paid and saved by each strategy.",
      noStrategyBaseline: "No strategy baseline",
      offsetOnlyTitle: "Offset account only",
      extraOnlyTitle: "Extra repayment only",
      lumpOnlyTitle: "One-off lump sum only",
      ...
      payoffSelected: "Pay-off date (selected)",
      timeSaved: "Time saved",
      footerTitle: "Mortgage Planner",
      footerTagline: "Plan today. Enjoy tomorrow.",
      monthly: "Monthly",
      fortnightly: "Fortnightly",
      weekly: "Weekly",
      yearly: "Yearly",
    ...
    fa: {
      ...
      monthlyBreakdownTitle: "جزئیات پرداخت ماهانه",
      monthlyBreakdownSub: "بر اساس گزینه‌های راهبردی انتخاب‌شده.",
      repayment: "مبلغ پرداخت",
      thisMonthInterest: "سود این ماه",
      thisMonthPrincipal: "اصل وام این ماه",
      interestSavedThisMonth: "سود صرفه‌جویی‌شده این ماه",
      afterSelectedStrategy: "پس از راهبرد انتخاب‌شده",
      interestComparisonTitle: "مقایسه سود",
      interestComparisonSub: "مقدار سود پرداختی و صرفه‌جویی‌شده را برای هر راهبرد مقایسه کنید.",
      noStrategyBaseline: "بدون راهبرد",
      offsetOnlyTitle: "فقط حساب آفست",
      ...
      payoffSelected: "تاریخ ختم وام با راهبرد انتخاب‌شده",
      timeSaved: "زمان صرفه‌جویی‌شده",
      footerTitle: "برنامه‌ریز وام خانه",
      footerTagline: "امروز برنامه‌ریزی کنید. فردا آسوده‌تر باشید.",
      monthly: "ماهانه",
      fortnightly: "هر دو هفته",
      weekly: "هفتگی",
      yearly: "سالانه",
  ...
  function applyLanguage(lang) {
    const t = I18N_FIX[lang] || I18N_FIX.en;
    ...
    setText(".brand-text", t.brand);
    setText("#headerDonationText", t.support);
    setHTML(".hero-overlay-copy h1", t.heroTitle);
    setText(".hero-overlay-copy .lead", t.heroLead);
    setText("#ctaPrimary span", t.ctaPrimary);
    setText("#ctaSecondary span", t.ctaSecondary);

    const benefits = document.querySelectorAll(".benefit strong");
    [t.benefit1, t.benefit2, t.benefit3].forEach((value, index) => {
      if (benefits[index]) benefits[index].textContent = value;
    });

    const statLabels = document.querySelectorAll(".stats-band .stat small");
    [t.statLoan, t.statRate, t.statTerm, t.statFrequency, t.statPayment].forEach((value, index) => {
      if (statLabels[index]) statLabels[index].textContent = value;
    });

    const panels = document.querySelectorAll(".panel");
    if (panels[0]) {
      const h2 = panels[0].querySelector("h2");
      const sub = panels[0].querySelector(".panel-sub");
      if (h2) h2.textContent = t.calcTitle;
      if (sub) sub.textContent = t.calcSub;
    }
    if (panels[1]) {
      const h2 = panels[1].querySelector("h2");
      const sub = panels[1].querySelector(".panel-sub");
      if (h2) h2.textContent = t.strategiesTitle;
      if (sub) sub.textContent = t.strategiesSub;
    }

    const loanLabels = document.querySelectorAll(".panel:first-child .field label");
    [t.loanStartDate, t.currentLoanBalance, t.interestRate, t.amortizationPeriod, t.repaymentFrequency].forEach((value, index) => {
      if (loanLabels[index]) loanLabels[index].textContent = value;
    });
    ...
    const strategies = document.querySelectorAll(".strategy");
    const strategyData = [
      [t.offsetTitle, t.offsetDesc, [t.currentOffset, t.growOffsetBy, t.growFrequency]],
      [t.extraTitle, t.extraDesc, [t.extraAmount, t.howOften]],
      [t.lumpTitle, t.lumpDesc, [t.lumpAmount]]
    ];
    ...
    strategies.forEach((strategy, index) => {
      ...
      const h3 = strategy.querySelector("h3");
      const p = strategy.querySelector("p");
      if (h3) h3.textContent = data[0];
      if (p) p.textContent = data[1];

      const useCheck = strategy.querySelector(".use-check");
      if (useCheck) {
        const nodes = Array.from(useCheck.childNodes);
        const textNode = nodes.find(node => node.nodeType === Node.TEXT_NODE);
        if (textNode) textNode.textContent = " " + t.use;
      }

      const labels = strategy.querySelectorAll(".field label");
      data[2].forEach((value, labelIndex) => {
    ...
    document.querySelectorAll("#repaymentFrequency option, #offsetFrequency option, #extraFrequency option").forEach(option => {
      if (selectLabels[option.value]) option.textContent = selectLabels[option.value];
    });

    document.querySelectorAll("#loanTerm option").forEach(option => {
      const years = option.value;
      option.textContent = years + " " + t.years;
    });

    setText(".monthly-breakdown h2", t.monthlyBreakdownTitle);
    setText(".monthly-breakdown .panel-sub", t.monthlyBreakdownSub);
    setText(".monthly-breakdown .pill", t.afterSelectedStrategy);

    const monthlyCards = document.querySelectorAll(".breakdown-card small");
    [t.repayment, t.thisMonthInterest, t.thisMonthPrincipal, t.interestSavedThisMonth].forEach((value, index) => {
      if (monthlyCards[index]) monthlyCards[index].textContent = value;
    });

    setText(".comparison h2", t.interestComparisonTitle);
    setText(".comparison .panel-sub", t.interestComparisonSub);
    setText("#chartTitle", t.balanceChartTitle);

    const compareCards = document.querySelectorAll(".compare-card");
    const compareTitles = [t.noStrategyBaseline, t.offsetOnlyTitle, t.extraOnlyTitle, t.lumpOnlyTitle, t.allSelectedTitle];
    compareCards.forEach((card, index) => {
      const h3 = card.querySelector("h3");
      if (h3 && compareTitles[index]) h3.textContent = compareTitles[index];

      const smalls = card.querySelectorAll("small");
      if (smalls[0]) smalls[0].textContent = t.interestYouPay;
      if (smalls[1]) smalls[1].textContent = t.interestYouSave;
    });

    const payoffLabels = document.querySelectorAll(".payoff-strip small");
    [t.payoffNoStrategy, t.payoffSelected, t.timeSaved].forEach((value, index) => {
      if (payoffLabels[index]) payoffLabels[index].textContent = value;
    });

    setText(".footer-title", t.footerTitle);
    setText(".footer-tagline", t.footerTagline);

    localStorage.setItem("mortgagePlannerLanguage", lang);
  }

  document.addEventListener("DOMContentLoaded", function () {
    const saved = localStorage.getItem("mortgagePlannerLanguage");
    const startLang = ["en", "fa"].includes(saved) ? saved : "en";
...
/* === Reliable six-language switch override === */
(function () {
  const I18N_FIX = {
  "en": {
    "dir": "ltr",
  ...
  "zh": {
    ...
    "brand": "房贷规划器",
    "support": "支持我们",
    "heroTitle": "更<span>聪明地规划。</span><br>更早还清房贷。",
    "heroLead": "强大的规划工具和清晰的数据分析，帮助你省钱、减少利息，并实现财务自由。",
    "ctaPrimary": "开始使用",
    "ctaSecondary": "了解如何运作",
    "benefit1": "查看还清日期",
    "benefit2": "节省数千元",
    ...
    "currentLoanBalance": "当前贷款余额",
    "interestRate": "利率",
    "amortizationPeriod": "还款期限",
    "repaymentFrequency": "还款频率",
    "dateHint": "日 / 月 / 年",
    "strategiesTitle": "策略选项",
    "strategiesSub": "选择一个或多个选项来比较策略。",
    "offsetTitle": "增加抵消账户",
    "offsetDesc": "减少需要支付的利息。",
    "extraTitle": "额外还款",
    "extraDesc": "向贷款本金额外还款。",
    "lumpTitle": "一次性还款",
    "lumpDesc": "一次性额外还款。",
    "use": "使用",
    "currentOffset": "当前抵消 / redraw",
    "growOffsetBy": "抵消账户增加金额",
    "growFrequency": "增加频率",
    "extraAmount": "额外金额",
    "howOften": "频率",
    "lumpAmount": "一次性金额",
    "monthlyBreakdownTitle": "月度还款明细",
    "monthlyBreakdownSub": "基于当前选择的策略选项。",
    "repayment": "还款额",
    "thisMonthInterest": "本月利息",
    "thisMonthPrincipal": "本月本金",
    "interestSavedThisMonth": "本月节省利息",
    "afterSelectedStrategy": "应用所选策略后",
    "interestComparisonTitle": "利息对比",
    "interestComparisonSub": "比较每种策略的已付利息和节省利息。",
    "noStrategyBaseline": "无策略基准",
    "offsetOnlyTitle": "仅抵消账户",
    "extraOnlyTitle": "仅额外还款",
    "lumpOnlyTitle": "仅一次性还款",
    "allSelectedTitle": "所有已选策略",
    "interestYouPay": "你支付的利息",
    "interestYouSave": "你节省的利息",
    "balanceChartTitle": "房贷余额变化",
    "payoffNoStrategy": "还清日期（无策略）",
    "payoffSelected": "还清日期（所选策略）",
    "timeSaved": "节省时间",
    "footerTitle": "房贷规划器",
    "footerTagline": "今天规划，明天更轻松。",
    "monthly": "每月",
    "fortnightly": "每两周",
    "weekly": "每周",
    "yearly": "每年",
  ...
  "hi": {
    ...
    "brand": "मॉर्गेज प्लानर",
    "support": "सहयोग करें",
    "heroTitle": "<span>समझदारी से</span> योजना बनाएं।<br>अपना होम लोन जल्दी चुकाएं।",
    "heroLead": "साफ़ जानकारी और उपयोगी योजना टूल्स, ताकि आप पैसे बचा सकें, ब्याज कम कर सकें और आर्थिक स्वतंत्रता की ओर बढ़ सकें।",
    "ctaPrimary": "शुरू करें",
    "ctaSecondary": "यह कैसे काम करता है",
    "benefit1": "अंतिम भुगतान तारीख देखें",
    "benefit2": "हज़ारों बचाएं",
    "benefit3": "ट्रैक पर रहें",
    "statLoan": "वर्तमान लोन बैलेंस",
    "statRate": "ब्याज दर",
    "statTerm": "अमॉर्टाइज़ेशन अवधि",
    "statFrequency": "भुगतान आवृत्ति",
    "statPayment": "भुगतान",
    "calcTitle": "मॉर्गेज कैलकुलेटर",
    "calcSub": "भुगतान समय का अनुमान लगाने के लिए अपने लोन विवरण दर्ज करें।",
    ...
    "currentLoanBalance": "वर्तमान लोन बैलेंस",
    "interestRate": "ब्याज दर",
    "amortizationPeriod": "अमॉर्टाइज़ेशन अवधि",
    "repaymentFrequency": "भुगतान आवृत्ति",
    "dateHint": "दिन / महीना / साल",
    "strategiesTitle": "रणनीति विकल्प",
    "strategiesSub": "रणनीतियों की तुलना के लिए एक या अधिक विकल्प चुनें।",
    "offsetTitle": "ऑफसेट खाता बढ़ाएं",
    "offsetDesc": "लगने वाला ब्याज कम करता है।",
    "extraTitle": "अतिरिक्त भुगतान",
    "extraDesc": "लोन मूलधन में अतिरिक्त भुगतान करें।",
    "lumpTitle": "एकमुश्त भुगतान",
    "lumpDesc": "एक बार का अतिरिक्त भुगतान।",
    "use": "उपयोग करें",
    "currentOffset": "वर्तमान ऑफसेट / redraw",
    "growOffsetBy": "ऑफसेट बढ़ाएं",
    "growFrequency": "बढ़ाने की आवृत्ति",
    "extraAmount": "अतिरिक्त राशि",
    "howOften": "कितनी बार",
    "lumpAmount": "एकमुश्त राशि",
    "monthlyBreakdownTitle": "मासिक भुगतान विवरण",
    "monthlyBreakdownSub": "वर्तमान चयनित रणनीतियों के आधार पर।",
    "repayment": "भुगतान",
    "thisMonthInterest": "इस महीने ब्याज",
    "thisMonthPrincipal": "इस महीने मूलधन",
    "interestSavedThisMonth": "इस महीने बचा ब्याज",
    "afterSelectedStrategy": "चयनित रणनीति के बाद",
    "interestComparisonTitle": "ब्याज तुलना",
    "interestComparisonSub": "हर रणनीति में भुगतान और बचत हुए ब्याज की तुलना करें।",
    "noStrategyBaseline": "बिना रणनीति आधार",
    "offsetOnlyTitle": "केवल ऑफसेट खाता",
    "extraOnlyTitle": "केवल अतिरिक्त भुगतान",
    "lumpOnlyTitle": "केवल एकमुश्त भुगतान",
    "allSelectedTitle": "सभी चयनित रणनीतियां",
    "interestYouPay": "आप जो ब्याज देंगे",
    "interestYouSave": "आप जो ब्याज बचाएंगे",
    "balanceChartTitle": "समय के साथ मॉर्गेज बैलेंस",
    "payoffNoStrategy": "अंतिम भुगतान तारीख (बिना रणनीति)",
    "payoffSelected": "अंतिम भुगतान तारीख (चयनित रणनीति)",
    "timeSaved": "बचा समय",
    "footerTitle": "मॉर्गेज प्लानर",
    "footerTagline": "आज योजना बनाएं। कल बेहतर जिएं।",
    "monthly": "मासिक",
    "fortnightly": "हर दो सप्ताह",
    "weekly": "साप्ताहिक",
    "yearly": "वार्षिक",
    "years": "साल"
  },
  "ur": {
    ...
    "heroTitle": "بہتر <span>منصوبہ بندی کریں۔</span><br>اپنا ہوم لون جلدی ختم کریں۔",
    "heroLead": "آسان اور واضح planning tools جو آپ کو پیسے بچانے، سود کم کرنے، اور مالی آزادی حاصل کرنے میں مدد دیتے ہیں۔",
    "ctaPrimary": "شروع کریں",
    "ctaSecondary": "یہ کیسے کام کرتا ہے",
    "benefit1": "ادائیگی ختم ہونے کی تاریخ دیکھیں",
    "benefit2": "ہزاروں بچائیں",
    "benefit3": "ٹریک پر رہیں",
    "statLoan": "موجودہ loan balance",
    "statRate": "سود کی شرح",
    "statTerm": "ادائیگی کی مدت",
    "statFrequency": "ادائیگی کی فریکوئنسی",
    "statPayment": "ادائیگی",
    "calcTitle": "مورگیج کیلکولیٹر",
    "calcSub": "ادائیگی مکمل ہونے کا وقت جاننے کے لیے loan details درج کریں۔",
    "loanStartDate": "لون شروع ہونے کی تاریخ",
    "currentLoanBalance": "موجودہ loan balance",
    "interestRate": "سود کی شرح",
    "amortizationPeriod": "ادائیگی کی مدت",
    "repaymentFrequency": "ادائیگی کی فریکوئنسی",
    "dateHint": "دن / مہینہ / سال",
    ...
    "strategiesSub": "حکمت عملیوں کا موازنہ کرنے کے لیے ایک یا زیادہ اختیارات منتخب کریں۔",
    "offsetTitle": "آفسیٹ اکاؤنٹ بڑھائیں",
    "offsetDesc": "چارج ہونے والا سود کم کرتا ہے۔",
    "extraTitle": "اضافی ادائیگی",
    "extraDesc": "loan principal کی طرف اضافی ادائیگی کریں۔",
    "lumpTitle": "ایک بار کی رقم",
    "lumpDesc": "ایک بار کی اضافی ادائیگی۔",
    "use": "استعمال کریں",
    "currentOffset": "موجودہ offset / redraw",
    "growOffsetBy": "offset میں اضافہ",
    "growFrequency": "اضافے کی فریکوئنسی",
    "extraAmount": "اضافی رقم",
    "howOften": "کتنی بار",
    "lumpAmount": "ایک بار کی رقم",
    "monthlyBreakdownTitle": "ماہانہ ادائیگی کی تفصیل",
    "monthlyBreakdownSub": "موجودہ منتخب حکمت عملیوں کی بنیاد پر۔",
    "repayment": "ادائیگی",
    "thisMonthInterest": "اس ماہ سود",
    "thisMonthPrincipal": "اس ماہ اصل رقم",
    "interestSavedThisMonth": "اس ماہ بچایا گیا سود",
    "afterSelectedStrategy": "منتخب حکمت عملی کے بعد",
    "interestComparisonTitle": "سود کا موازنہ",
    "interestComparisonSub": "ہر حکمت عملی کے مطابق ادا شدہ اور بچائے گئے سود کا موازنہ کریں۔",
    "noStrategyBaseline": "بغیر حکمت عملی",
    "offsetOnlyTitle": "صرف offset account",
    "extraOnlyTitle": "صرف اضافی ادائیگی",
    "lumpOnlyTitle": "صرف ایک بار کی رقم",
    "allSelectedTitle": "تمام منتخب حکمت عملیاں",
    "interestYouPay": "آپ جو سود ادا کریں گے",
    "interestYouSave": "آپ جو سود بچائیں گے",
    "balanceChartTitle": "وقت کے ساتھ mortgage balance",
    "payoffNoStrategy": "ادائیگی ختم ہونے کی تاریخ (بغیر حکمت عملی)",
    "payoffSelected": "ادائیگی ختم ہونے کی تاریخ (منتخب حکمت عملی)",
    "timeSaved": "بچا ہوا وقت",
    "footerTitle": "مورگیج پلانر",
    "footerTagline": "آج منصوبہ بنائیں۔ کل بہتر گزاریں۔",
    "monthly": "ماہانہ",
    "fortnightly": "ہر دو ہفتے",
    "weekly": "ہفتہ وار",
    "yearly": "سالانہ",
    "years": "سال"
  },
  "es": {
    "dir": "ltr",
    "lang": "es",
    "brand": "Planificador Hipotecario",
    "support": "Apóyanos",
    "heroTitle": "Planifica <span>mejor.</span><br>Paga tu hipoteca antes.",
    "heroLead": "Herramientas claras y potentes para ayudarte a ahorrar dinero, reducir intereses y alcanzar libertad financiera.",
    "ctaPrimary": "Empezar",
    "ctaSecondary": "Cómo funciona",
    "benefit1": "Mira tu fecha final",
    "benefit2": "Ahorra miles",
    "benefit3": "Mantente al día",
    "statLoan": "Saldo actual del préstamo",
    "statRate": "Tasa de interés",
    "statTerm": "Plazo de amortización",
    "statFrequency": "Frecuencia de pago",
    "statPayment": "Pago",
    "calcTitle": "Calculadora hipotecaria",
    "calcSub": "Ingresa los datos de tu préstamo para estimar cuándo lo pagarás.",
    "loanStartDate": "Fecha de inicio",
    "currentLoanBalance": "Saldo actual del préstamo",
    "interestRate": "Tasa de interés",
    "amortizationPeriod": "Plazo de amortización",
    "repaymentFrequency": "Frecuencia de pago",
    "dateHint": "Día / Mes / Año",
    "strategiesTitle": "Opciones de estrategia",
    "strategiesSub": "Elige una o más opciones para comparar estrategias.",
    "offsetTitle": "Aumentar cuenta offset",
    "offsetDesc": "Reduce los intereses cobrados.",
    "extraTitle": "Pago adicional",
    "extraDesc": "Paga extra hacia el capital del préstamo.",
    "lumpTitle": "Pago único",
    "lumpDesc": "Un pago adicional de una sola vez.",
    "use": "Usar",
    "currentOffset": "Offset / redraw actual",
    "growOffsetBy": "Aumentar offset en",
    "growFrequency": "Frecuencia de aumento",
    "extraAmount": "Monto adicional",
    "howOften": "Frecuencia",
    "lumpAmount": "Monto único",
    "monthlyBreakdownTitle": "Desglose del pago mensual",
    "monthlyBreakdownSub": "Basado en las estrategias seleccionadas.",
    "repayment": "Pago",
    "thisMonthInterest": "Interés este mes",
    "thisMonthPrincipal": "Capital este mes",
    "interestSavedThisMonth": "Interés ahorrado este mes",
    "afterSelectedStrategy": "Después de la estrategia",
    "interestComparisonTitle": "Comparación de intereses",
    "interestComparisonSub": "Compara los intereses pagados y ahorrados por cada estrategia.",
    "noStrategyBaseline": "Base sin estrategia",
    "offsetOnlyTitle": "Solo cuenta offset",
    "extraOnlyTitle": "Solo pago adicional",
    "lumpOnlyTitle": "Solo pago único",
    "allSelectedTitle": "Todas las estrategias seleccionadas",
    "interestYouPay": "Interés que pagas",
    "interestYouSave": "Interés que ahorras",
    "balanceChartTitle": "Saldo hipotecario en el tiempo",
    "payoffNoStrategy": "Fecha de pago final (sin estrategia)",
    "payoffSelected": "Fecha de pago final (seleccionada)",
    "timeSaved": "Tiempo ahorrado",
    "footerTitle": "Planificador Hipotecario",
    "footerTagline": "Planifica hoy. Disfruta mañana.",
    "monthly": "Mensual",
    "fortnightly": "Quincenal",
    "weekly": "Semanal",
    "yearly": "Anual",
  ...
  const SUPPORTED_LANGS = Object.keys(I18N_FIX);
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const saved = localStorage.getItem("mortgagePlannerLanguage");
    const startLang = SUPPORTED_LANGS.includes(saved) ? saved : "en";
    applyLanguage(startLang);
